...
function classifyWaste(text: string): {
  bin: "Green" | "Silver" | "Blue" | "Brown";
  instruction: string;
} {
  ...
  if (
    ...
    return {
      bin: "Brown",
      instruction: "Compostable Bin: Place food leftovers here.",
    };
  }
  if (
    ...
    return {
      bin: "Blue",
      instruction: "Recycling Bin: Empty liquids first before tossing.",
    };
  }
  if (
    ...
    return {
      bin: "Green",
      instruction: "Paper Bin: Ensure paper is dry and non-greasy.",
    };
  }
  return {
    bin: "Silver",
    instruction: "General Landfill Waste: Toss non-recyclable items here.",
  };
}

// AI Hardcoded Response Matrix
function getAiResponse(
  role: "fan" | "organizer" | "volunteer",
  query: string,
  _lang = "en",
): string {
...
const initialIncidents: Incident[] = [
  {
    id: "INC-101",
    description: "Crowd slowdown reported near outer terminal ramp",
    severity: "Medium",
    status: "In Progress",
  ...
  {
    id: "INC-102",
    description: "Minor liquid spill near concession counter row B",
    severity: "Low",
    status: "Pending",
...
const initialTasks: AppState["tasks"] = [
  {
    id: "TSK-01",
    label: "Escort family with registered wheelchair access requirements",
    priority: "High",
    status: "pending",
  },
  {
    id: "TSK-02",
    label: "Distribute tournament sustainability pamphlets at main concourse",
    priority: "Low",
    status: "pending",
  },
  {
    id: "TSK-03",
    label: "Monitor Gate B perimeter overspill metrics",
    priority: "High",
    status: "in_progress",
  },
];
...
const GenAiBadge = () => (
  <span
    ...
    className="text-xs font-bold inline-flex items-center gap-1 tracking-wider uppercase select-none"
  >
    ✨ Powered by GenAI
  </span>
);
      ...
      return (
        <div className="p-8 text-center bg-red-900/20 border border-red-500/40 rounded-xl max-w-lg mx-auto my-12">
          <h3 className="text-xl font-bold text-white mb-2">
            Operational Boundary Failure
          </h3>
          <p className="text-gray-300 text-sm mb-4">
            A critical display view error occurred in this module component
            tree.
          </p>
          <button
            onClick={() => this.setState({ hasError: false })}
            className="px-5 py-2 bg-red-600 hover:bg-red-700 text-white font-medium rounded-lg text-sm transition-colors siq-focusable"
          >
            Reset Module View
          </button>
        </div>
...
function SkeletonDisplayGrid({ isDark }: { isDark: boolean }) {
  return (
    <div className="space-y-4">
      ...
      <div
        className={`h-40 rounded-xl border p-4 animate-pulse ${isDark ? "bg-slate-900 border-slate-700" : "bg-white border-slate-200"}`}
      />
    </div>
  );
}
...
export function App() {
    ...
    const wsInterval = setInterval(() => {
      ...
      if (rand > 0.7) {
        dispatch({
          ...
          payload: `[WS Alert] Flow rate optimized at Gate ${Math.random() > 0.5 ? "A" : "C"}.`,
        });
      } else if (rand < 0.25) {
        const newInc: Incident = {
          id: `INC-${Math.floor(100 + Math.random() * 900)}`,
          description:
            "Spike flagged via operational crowd density telemetry trackers",
  ...
  return (
    <div
      id="main-content"
      className={`stadiumiq-root min-h-screen relative overflow-hidden flex flex-col ${
        isAccess
          ? "bg-black text-white text-lg font-bold"
          : isDark
            ? "bg-[#070b16] text-[#f3f6fa]"
            : "bg-[#f0f4fa] text-[#0f1a30]"
      }`}
    >
      ...
      {transitionTarget && (
        <div
          role="presentation"
          className="fixed inset-0 z-50 pointer-events-none flex items-center bg-black/10 backdrop-blur-[1px]"
        >
      ...
      {!isOnline && (
        <div
          className="bg-yellow-600 text-black text-center text-xs font-bold py-1 px-4 z-50 animate-fade-in"
          role="alert"
        >
          Offline Mode Active. Cached operational schemas will synchronize
          dynamically upon reconnection.
        </div>
      )}
      ...
      {state.lastBroadcast && (
        <div
          className="bg-blue-900 border-b border-blue-700 text-white text-xs px-4 py-2 flex justify-between items-center gap-2 font-medium z-30"
          aria-live="assertive"
        >
          ...
          <button
            onClick={() => dispatch({ type: "RECEIVE_BROADCAST", payload: "" })}
            className="text-white/70 hover:text-white font-bold text-sm px-1 rounded siq-focusable"
      ...
      <header
        style={{ backgroundColor: "rgba(7, 11, 22, 0.94)" }}
        className="text-[#f3f6fa] border-b border-gray-800 z-40 sticky top-0 backdrop-blur-md"
      >
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="bg-slate-900 border border-slate-700/60 rounded px-2.5 py-1 flex items-center gap-2 text-xs font-semibold">
              ...
              <span className="text-white font-mono bg-slate-950 px-1.5 rounded">
                2 - 1
              </span>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {state.role !== "onboarding" && (
              <div className="flex items-center gap-1.5 bg-slate-900/80 border border-slate-700 p-1 rounded">
                <label htmlFor="global-role-switcher" className="sr-only">
                  Switch Platform Persona View
                </label>
                <select
                  id="global-role-switcher"
                  value={state.role}
                  onChange={(e) =>
                    handleRoleTransition(e.target.value as AppState["role"])
                  }
        ...
        <div
          className="h-[28px] bg-slate-950 border-t border-slate-900 flex items-center"
          role="presentation"
        >
          ...
          <div className="flex-1 overflow-hidden h-full flex items-center relative">
            <div className="animate-marquee whitespace-nowrap flex items-center gap-12 text-xs font-medium tracking-wide text-[#abc0d8]">
              ...
              <span>
                Match 82 | Group Stage | Gate B now open — reduced wait time 2.4
                min
              </span>
              <span>
              ...
              <span>
                Shuttle Service: Platform 3 departing in 8 min — Gate C exit
                recommended
              </span>
            </div>
          </div>
      ...
      <footer
        className={`mt-auto border-t py-4 text-center text-xs transition-colors ${
          isAccess
            ...
            : isDark
              ? "bg-[#0e162b] border-gray-800 text-[#6c829e]"
              : "bg-white border-gray-200 text-[#485a7e]"
        }`}
      >
        <div className="px-4 flex flex-col sm:flex-row justify-between items-center gap-2 max-w-7xl mx-auto">
          <p>
            © 2026 FIFA Operations Dashboard Hub — StadiumIQ Systems Ecosystem.
          </p>
...
function RoleSelectionLanding({
  dispatch,
  isDark,
  isAccess,
}: {
  dispatch: (role: AppState["role"]) => void;
  isDark: boolean;
  isAccess: boolean;
}) {
  ...
  const cards = [
    {
      id: "fan",
      label: "Fan Portal",
      desc: "Your game-day companion for navigation, AI assistance, and eco rewards",
      theme: "border-amber-500/20",
      icon: "🎫",
    },
    {
      id: "organizer",
      label: "Command Center",
      desc: "Command Center for real-time crowd intelligence and operations control",
      theme: "border-blue-500/20",
      icon: "🎛️",
    },
    {
      id: "volunteer",
      label: "Volunteer / Staff",
      desc: "Your shift briefing, task board, and SOP assistant",
      theme: "border-emerald-500/20",
      icon: "📋",
    },
  ];
  ...
  return (
    <div className="relative py-8 px-2 flex flex-col items-center justify-center min-h-[75vh]">
      ...
      {!isAccess && (
        <div
          role="presentation"
          className="absolute inset-0 z-0 pointer-events-none opacity-40 overflow-hidden rounded-2xl bg-[#001a4e]/10 border border-[#c9a227]/15"
        >
          <svg
          ...
          {Array.from({ length: ANIMATION_CONSTANTS.BALL_COUNT }).map(
            (_, idx) => {
              const positions = [
      ...
      <div className="z-10 text-center mb-10 max-w-xl">
        ...
        <p
          className={`text-sm ${isAccess ? "text-white" : "text-[#485a7e]"} max-w-md mx-auto`}
        >
          Select operational mode node parameters to launch real-time
          coordination vectors inside MetLife Stadium infrastructure matrices.
        </p>
      </div>
      ...
      <div className="z-10 grid grid-cols-1 md:grid-cols-3 gap-6 w-full max-w-5xl px-4">
        {cards.map((c) => (
          ...
          >
            <div>
              <div className="text-3xl mb-3" role="presentation">
                {c.icon}
              </div>
              <h3 className="text-xl font-bold mb-2 tracking-tight group-hover:text-[#c9a227] transition-colors">
                {c.label}
              </h3>
              <p
                className={`text-xs mb-6 leading-relaxed ${isAccess ? "text-white" : isDark ? "text-[#abc0d8]" : "text-[#485a7e]"}`}
              >
                {c.desc}
              </p>
            </div>
            ...
            {!isAccess && hoveredCard === c.id && (
              <div
                className="animate-card-hover-ball absolute bottom-3 right-3 opacity-90"
                role="presentation"
              >
...
function FanPortalView({
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
      ...
      <nav
        className="lg:col-span-1 flex flex-row lg:flex-col gap-1 overflow-x-auto lg:overflow-x-visible pb-2 lg:pb-0 border-b lg:border-b-0 lg:border-r border-slate-700/20"
        aria-label="Fan Portal Navigation Sub-tabs"
      >
        {tabs.map((t) => (
          <button
            key={t.id}
            ...
            className={`flex items-center gap-2 px-3 py-2.5 rounded-lg text-xs font-bold whitespace-nowrap tracking-wide transition-all ${
              activeTab === t.id
                ? isAccess
                  ...
                  : "bg-[#c9a227] text-[#001a4e] shadow-sm"
                : isAccess
                  ? "bg-black text-white border border-white hover:bg-slate-900"
                  : isDark
      ...
      <div className="lg:col-span-4 min-h-[60vh]">
        ...
        {activeTab === "navigate" && (
          <FanTabNavigate state={state} isDark={isDark} isAccess={isAccess} />
        )}
        {activeTab === "ai" && <FanTabAi isDark={isDark} isAccess={isAccess} />}
        {activeTab === "ticket" && (
          <FanTabTicket isDark={isDark} isAccess={isAccess} />
        )}
        {activeTab === "eco" && (
          <FanTabEco
            state={state}
...
function FanTabHome({
  ...
  const [accWheelchair, setAccWheelchair] = useState(
    state.accessibilityNeeds.wheelchair,
  );
  ...
  const handleExitPlanSubmit = (e: React.FormEvent) => {
    ...
    setExitPlanResult(
      `[AI-Generated] Optimized Routing Configuration Matrix identified: Exit via Gate C (lowest current congestion metric index: 2.4 min wait) → Board NJ Transit Shuttle Express directly at Meadowlands Station Terminal Platform 2 → Projected destination ETA: 48 minutes total.`,
    );
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div
          className={`p-4 border rounded-xl ${isAccess ? "border-red-600 bg-black" : "bg-red-950/20 border-red-500/30"}`}
        >
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2 py-0.5 bg-red-600 text-white font-bold rounded text-[10px] tracking-wide uppercase">
          ...
          <p className="text-xs text-white">
            Crowd limits are reaching 94% density threshold flags. Use secondary
            peripheral exit routes via Gate C corridor segments.
          </p>
        </div>

        <div
          className={`p-4 border rounded-xl ${isAccess ? "border-green-600 bg-black" : "bg-green-950/20 border-green-500/30"}`}
        >
          <div className="flex items-center gap-2 mb-1">
            <span className="px-2 py-0.5 bg-green-600 text-white font-bold rounded text-[10px] tracking-wide uppercase">
              FAST TRACK OPEN
            </span>
            <span className="text-xs font-mono text-green-400">
              Gate B Security Lines
            </span>
          </div>
          <p className="text-xs text-white">
            Current entry/clearance tracking processing cycles sit optimal at
            ~2.4 mins total transit latency parameters.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div
          className={`p-5 rounded-xl border md:col-span-1 flex flex-col justify-between ${isDark ? "bg-[#0e162b] border-[#c9a227]/10" : "bg-white border-slate-200"}`}
        >
          <div>
            <h4 className="text-xs font-black text-[#c9a227] tracking-wider uppercase mb-3">
              Live Digital Token
            </h4>
            <div className="text-2xl font-black mb-1">MATCH 82</div>
            <div
              className={`text-xs ${isDark ? "text-gray-300" : "text-slate-600"} space-y-0.5 font-semibold`}
            >
          ...
          <button
            ...
            className="mt-4 w-full text-center py-2 bg-slate-800 text-white hover:bg-slate-700 rounded text-xs font-bold tracking-wide transition-colors"
          >
        ...
        <div
          className={`p-5 rounded-xl border md:col-span-2 ${isDark ? "bg-[#0e162b] border-[#c9a227]/10" : "bg-white border-slate-200"}`}
        >
          <div className="flex justify-between items-center mb-3">
            <h4 className="text-xs font-black text-[#c9a227] tracking-wider uppercase">
              Concession Wait Diagnostics
            </h4>
          ...
          <div className="space-y-2.5">
            {[
              {
                name: "Halal Bites Express (Sec 112)",
                wait: "3 min queue",
                status: "Optimal Flow",
                bg: "bg-green-600",
              },
              {
                name: "Jersey Burger Grills (Sec 204)",
                wait: "18 min queue",
                status: "Saturated",
                bg: "bg-red-600",
              },
              {
                name: "Trophy Tacos Hub (Sec 118)",
                wait: "7 min queue",
                status: "Normal Flow",
                bg: "bg-amber-600",
              },
              ...
              <div
                key={i}
                className="flex justify-between items-center text-xs border-b border-gray-700/20 pb-1.5 last:border-0"
              >
                <span className="font-semibold">{v.name}</span>
                <div className="flex items-center gap-2">
                  <span className="font-mono text-gray-400">{v.wait}</span>
                  <span
                    className={`px-1.5 py-0.5 rounded text-[9px] text-white font-bold ${v.bg}`}
                  >
                    {v.status}
                  </span>
                </div>
          ...
          <p className="mt-3 text-[11px] text-[#c9a227] font-semibold italic bg-amber-500/5 p-2 rounded border border-amber-500/10">
            💡 AI Operational Tip: Halal Bites queue is optimized. Avoid Jersey
            Burger segment channels until the 80th-minute operational reset
            windows.
          </p>
        </div>
      </div>
      ...
      <div
        className={`p-5 rounded-xl border ${isDark ? "bg-[#0e162b] border-[#c9a227]/10" : "bg-white border-slate-200"}`}
      >
        <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
          <div>
            ...
            <p className="text-xs text-gray-400">
              Calculate high-efficiency escape trajectories mitigating active
              stadium outer congestion trends.
            </p>
          </div>
          <button
        ...
        {showExitPlanner && (
          <form
            onSubmit={handleExitPlanSubmit}
            className="space-y-4 border-t border-gray-700/20 pt-4"
          >
            <div>
              <label
                htmlFor="exit-dest-input"
                className="block text-xs font-bold mb-1 text-gray-300"
              >
                Terminal Transit Target Destination
              </label>
              <div className="flex gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setExitDestination(sanitizeText(e.target.value))
                  }
                ...
                <button
                  type="submit"
                  className="px-4 py-1.5 bg-[#c9a227] text-[#001a4e] font-bold text-xs rounded-lg hover:bg-opacity-90 transition-all"
                >
                  Compute Vectors
                </button>
              </div>
      ...
      >
        <h4 className="text-sm font-bold tracking-tight mb-2">
          ♿ Adaptive Accessibility Registration Matrix
        </h4>
        <p className="text-xs text-gray-400 mb-4">
          Flags configured here dynamically pass parameters into live supervisor
          dispatch modules for ground tactical support.
        </p>
        ...
        <div className="flex flex-col gap-2.5 max-w-md">
          ...
          {registeredSuccess && (
            <div className="text-xs text-green-500 font-bold mt-1">
              ✓ Settings operationalized. Ground staff briefing metrics synced.
            </div>
          )}
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center bg-slate-900/40 p-3 rounded-xl border border-slate-700/30 flex-wrap gap-2">
        <div>
          ...
          <p className="text-[11px] text-gray-400">
            Simulating live spatial matrix calculation vectors over MetLife
            coordinates.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div
          className={`md:col-span-2 border rounded-xl p-4 flex flex-col items-center justify-center min-h-[340px] ${isDark ? "bg-slate-950 border-gray-800" : "bg-slate-100 border-gray-200"}`}
        >
          <div className="w-full text-left mb-2 text-[10px] font-mono tracking-wider text-gray-400">
            METLIFE SECTOR SCENARIO MATRIX LAYOUT
          </div>
          ...
          <div className="relative w-full max-w-[400px] h-[300px] bg-slate-900/90 rounded-xl border border-slate-700 overflow-hidden shadow-inner">
            <svg
              className="w-full h-full"
              viewBox="0 0 400 300"
              aria-label="Stadium map schematic outlining paths and zones"
            >
              {/* Perimeter Rings */}
              <circle
                cx="200"
              ...
              {accessibleOverlay && (
                <path
                  d="M 80 150 Q 140 70 200 150 T 320 150"
                  fill="none"
                  stroke="#a855f7"
                  strokeWidth="3"
                  strokeDasharray="6 4"
              ...
              <circle
                cx={userCoordinates.x}
                cy={userCoordinates.y}
                r="7"
                fill="#3182ce"
                className="transition-all duration-300 ring-4 ring-blue-500/30"
              />
              <circle
                cx={userCoordinates.x}
                cy={userCoordinates.y}
            ...
            <div className="absolute bottom-2 left-2 bg-slate-950/90 text-[10px] font-mono p-1.5 rounded border border-slate-800 text-gray-300">
              User Vector Position: X: {userCoordinates.x}px | Y:{" "}
              {userCoordinates.y}px
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4 md:col-span-1 text-xs">
          <div
            className={`p-4 rounded-xl border ${isDark ? "bg-[#0e162b] border-[#c9a227]/10" : "bg-white border-slate-200"}`}
          >
            <h4 className="text-xs font-black text-[#c9a227] tracking-wider uppercase mb-3">
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2 border-t border-gray-700/20 font-mono text-[10.5px] space-y-1.5 text-gray-400">
                ...
                <div>
                  ...
                  <span className="text-white">100x100</span>
                </div>
              </div>
            </div>
          </div>
...
function FanTabAi({
  ...
  >([
    {
      id: "1",
      sender: "ai",
      text: "[AI-Generated] Welcome to MetLife Stadium. I am your World Cup AI Companion. How can I assist with your seating, food, or exit logistics today?",
    },
  ]);
  ...
  return (
    <div
      className={`rounded-xl border p-5 space-y-4 flex flex-col h-[500px] ${isDark ? "bg-[#0e162b] border-[#c9a227]/10" : "bg-white border-slate-200"}`}
    >
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-thin">
        {messages.map((m) => (
          ...
          >
            <div
              className={`max-w-[75%] rounded-xl px-3.5 py-2 text-xs font-semibold leading-relaxed ${
                m.sender === "user"
      ...
      <form onSubmit={handleSend} className="flex gap-2">
        ...
        <button
          type="submit"
          className="px-4 py-2 bg-[#c9a227] text-[#001a4e] font-black text-xs rounded-lg hover:bg-opacity-90 transition-all uppercase tracking-wider"
        >
          Send
        </button>
      </form>
      ...
      <details className="text-[9.5px] border-t border-gray-700/20 pt-2 text-gray-400 font-mono">
        <summary className="cursor-pointer font-bold text-amber-500/70 hover:text-amber-400">
          RAG Prompt & Embeddings Transparency Inspector
        </summary>
        <div className="mt-2 space-y-1 bg-slate-950 p-2 rounded border border-slate-800">
          <div>
            System Prompt Context Template:{" "}
            <span className="text-white">{"{stadium_faq_embeddings}"}</span>
          </div>
          <div>
            Retrieved Vector Score (Cosine Similarity):{" "}
            <span className="text-green-400 font-bold">0.8742</span>
          </div>
          <div>
            Matched pgvector segment ID:{" "}
            <span className="text-white font-bold">
              faq_metlife_concessions_112
            </span>
          </div>
        </div>
      </details>
...
function FanTabTicket({
  ...
  return (
    <div className="flex flex-col items-center justify-center p-8 text-center min-h-[400px]">
      <div
        className={`rounded-2xl border p-6 max-w-sm w-full space-y-5 shadow-xl relative overflow-hidden ${isDark ? "bg-[#0e162b] border-[#c9a227]/20 text-white" : "bg-white border-slate-200 text-slate-900"}`}
      >
        ...
        <div className="text-left border-b border-gray-700/20 pb-3 flex justify-between items-center">
          ...
          <span className="bg-[#00a651]/20 text-[#00c863] border border-[#00c863]/30 px-2 py-0.5 rounded text-[9px] font-bold uppercase">
            Verified
          </span>
        </div>
        ...
        <div className="space-y-1 font-mono text-xs text-left">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-bold text-[#e8c84a]">Gate B</span>
          </div>
        </div>
        ...
        <div className="bg-white p-3.5 rounded-lg flex flex-col items-center justify-center border border-slate-200">
          ...
          <span className="text-[8px] font-mono text-slate-800 tracking-widest mt-1">
            SIQ-FIFA2026-M82-SEC212
          </span>
        </div>

        <p className="text-[10px] text-gray-400">
          Keep screen brightness at max for seamless gate scanner operational
          processing.
        </p>
      </div>
    </div>
...
function FanTabEco({
  ...
  return (
    <div className="space-y-6">
      ...
      <GoalCelebration
        active={state.triggerLevelBurst}
        onComplete={() => dispatch({ type: "RESET_BURST" })}
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div
          className={`p-5 rounded-xl border relative overflow-hidden flex flex-col justify-between ${isDark ? "bg-[#0e162b] border-[#c9a227]/20" : "bg-white border-slate-200"}`}
        >
          <div>
            <h4 className="text-xs font-black text-[#c9a227] tracking-wider uppercase mb-2">
              XP Progress Matrix
            </h4>
            ...
            {/* Progress bar */}
            <div
              className="w-full h-2 rounded bg-slate-800 overflow-hidden mt-2"
              role="presentation"
            >
              <div
                className="h-full bg-gradient-to-r from-green-500 to-[#c9a227] transition-all duration-500"
        ...
        >
          <div>
            <h4 className="text-xs font-black text-[#00a651] tracking-wider uppercase mb-2">
              Carbon Footprint Live Counter
            </h4>
            <div className="text-2xl font-black text-[#00c863]">
              {1240 + state.ecoPoints * 0.45} kg CO₂{" "}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            >
              ...
              <div className="text-[#00c863] text-[10px] mt-1">
                +{action.pts} pts
              </div>
            </button>
          ))}
      ...
      >
        ...
        <form onSubmit={handleWasteSubmit} className="flex gap-2">
          ...
          <button
            type="submit"
            className="px-4 py-2 bg-green-600 text-white font-bold text-xs rounded-lg hover:bg-green-500 transition-all uppercase tracking-wider"
          >
            Classify
          </button>
        </form>
          ...
          <div className="mt-3 p-3 bg-slate-950 border border-green-500/20 rounded-lg flex items-center justify-between gap-3 text-xs">
            <div>
              <div className="font-bold text-white mb-0.5">
                Bin Classification Mapping Result
              </div>
            ...
            <span
              className={`px-3 py-1.5 rounded-lg text-black font-black uppercase text-[10px] tracking-wider ${
                wasteResult.bin === "Green"
                  ? "bg-green-400"
                  : wasteResult.bin === "Blue"
                    ? "bg-blue-400"
                    : wasteResult.bin === "Brown"
        ...
        >
          ...
          <div className="space-y-3">
            {[
              {
                id: "1",
                title: "Free Halal Hot Dog Coupon",
                cost: 100,
                code: "HDOG-FREE-2026",
              },
              {
                id: "2",
                title: "20% off Tournament Official Merchandise",
                cost: 150,
                code: "MERCH-20-SIQ",
              },
            ].map((v) => (
              <div
                key={v.id}
                className="flex justify-between items-center text-xs border-b border-gray-700/20 pb-2.5 last:border-0 last:pb-0"
              >
                <div>
                  <div className="font-bold">{v.title}</div>
                  <div className="text-[10px] text-gray-400">
                    {v.cost} Eco Points required
                  </div>
                </div>
                <button
          ...
          {showVoucher && (
            <div className="mt-4 p-4 bg-white border border-slate-200 rounded-lg text-black text-center space-y-3 animate-fade-in relative">
              <button
                ...
                className="absolute top-2 right-2 text-slate-500 font-bold hover:text-black"
              >
                ✕
              </button>
              <h5 className="font-bold text-sm">Voucher Activated!</h5>
              <div className="font-mono text-lg font-black bg-slate-100 py-1.5 rounded tracking-widest">
                {voucherCode}
              </div>
              ...
              <p className="text-[9px] text-slate-500">
                Scan at concession registers to redeem your voucher.
              </p>
            </div>
          )}
        </div>
        ...
        >
          ...
          <div className="space-y-2.5">
            {[
              ...
              <div
                key={usr.rank}
                className="flex justify-between items-center text-xs font-mono pb-1 border-b border-gray-700/10 last:border-0"
              >
                <div className="flex items-center gap-2">
                  <span className="font-bold text-[#c9a227] w-4">
                    {usr.rank}
                  </span>
                ...
                <div className="flex gap-4">
                  ...
                  <span className="text-[#00c863] font-bold">
                    {usr.pts} pts
                  </span>
                </div>
              </div>
...
function CommandCenterView({
  ...
  const medicalCount = openIncidents.filter(
  ...
  const handleEvacTrigger = () => {
    ...
    dispatch({
      type: "RECEIVE_BROADCAST",
      payload:
        "[CRITICAL DISPATCH] EVACUATION PROTOCOL ACTIVE. Clear Gate A and route outbound flow through Gate C.",
    });
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        className={`p-4 rounded-xl border flex items-center justify-between flex-wrap gap-4 ${isDark ? "bg-[#0e162b] border-[#c9a227]/20" : "bg-white border-slate-200"}`}
      >
        <div className="flex items-center gap-4 flex-wrap">
          <div>
            <h3 className="text-[10px] text-gray-400 font-bold uppercase tracking-wider">
              MetLife Match Clock
            </h3>
          ...
          <div>
            ...
            <span className="px-2 py-0.5 bg-[#00a651] text-white font-bold rounded text-[9px] uppercase tracking-wide">
        ...
        >
          <div className="bg-[#0e162b] border border-red-500 p-6 rounded-xl max-w-sm w-full text-center space-y-4 shadow-2xl">
            <h3 className="text-lg font-black text-red-500 uppercase tracking-wider">
              Activate Stadium Evacuation Plan
            </h3>
            <p className="text-xs text-gray-300 leading-relaxed">
              This will execute automated crowd redirect signals, broadcast
              emergency warning banners, and assign volunteers to safety paths.
            </p>
            <div className="flex gap-3">
              <button
                onClick={() => setEvacConfirm(false)}
                className="flex-1 py-2 bg-slate-800 text-white font-bold rounded text-xs hover:bg-slate-700 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleEvacTrigger}
                className="flex-1 py-2 bg-red-600 text-white font-bold rounded text-xs hover:bg-red-500 transition-colors"
              >
                Confirm
              </button>
            </div>
      ...
      {evacActive && (
        <div
          className="p-5 border border-red-500 bg-red-950/20 rounded-xl space-y-3 animate-fade-in"
          role="alert"
        >
          <h4 className="text-red-400 font-black text-sm uppercase tracking-widest flex items-center gap-2">
            🚨 [AI-Generated Evacuation Plan] — Execute Mode Live
          </h4>
          <p className="text-xs text-red-200">
            Recommended Sequenced Strategy: Open all Gate C routes immediately.
            Direct Concourse 200 staff volunteers to outer stairs. Projected
            clearance matrix: 18.5 minutes total.
          </p>
          <button
            onClick={() => setEvacActive(false)}
            className="px-3 py-1 bg-red-800 text-white text-[10px] font-bold rounded hover:bg-red-700"
          >
            Dismiss Plan
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div className="rounded-xl border border-slate-700/40 bg-slate-900/60 p-4 relative overflow-hidden flex flex-col justify-between min-h-[110px]">
          <div className="flex justify-between items-center mb-2 z-10">
            <span className="text-[9px] font-black uppercase text-[#6c829e] tracking-wider">
              Crowd Density
            </span>
            <span className="px-1.5 py-0.5 bg-amber-600 text-white font-black rounded text-[8px] tracking-wide uppercase">
              Warning
            </span>
          </div>
          <p className="font-display font-black text-3xl text-white z-10 mt-1">
            82%
          </p>
          <p className="text-[9px] text-[#6c829e] z-10">
            Live Overall Grid Occupancy
          </p>
        ...
        <div className="rounded-xl border border-slate-700/40 bg-slate-900/60 p-4 flex flex-col justify-between min-h-[110px]">
          ...
          <p className="font-display font-black text-3xl text-red-400 mt-1">
        ...
        <div className="rounded-xl border border-slate-700/40 bg-slate-900/60 p-4 flex flex-col justify-between min-h-[110px]">
          ...
          <p className="font-display font-black text-3xl text-[#c9a227] mt-1">
            34 / 50
          </p>
          <p className="text-[9.5px] text-gray-400 font-semibold mt-1">
        ...
        <div className="rounded-xl border border-slate-700/40 bg-slate-900/60 p-4 flex flex-col justify-between min-h-[110px]">
          <span className="text-[9px] font-black uppercase text-[#6c829e] tracking-wider mb-2">
            Queue Wait Times
          </span>
          <div className="space-y-1.5">
            {[
              ...
              <div
                key={g.gate}
                className="flex items-center gap-2 text-[8px] font-semibold text-gray-400"
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {/* Forecast Chart */}
        <div
          className={`md:col-span-2 p-5 rounded-xl border ${isDark ? "bg-[#0e162b] border-[#c9a227]/10" : "bg-white border-slate-200"}`}
        >
          <h4 className="text-xs font-black text-[#c9a227] tracking-wider uppercase mb-4">
            LSTM Crowd Surge Forecast (60 Min Projection)
          </h4>
          ...
          <div className="h-44 w-full flex items-center justify-center">
            ...
            >
              <path
                d="M 10 90 Q 40 40 80 80 T 150 20 L 190 60 L 190 90 Z"
                fill="#c9a227"
                fillOpacity="0.1"
                stroke="#c9a227"
                strokeWidth="2"
              ...
              <text
                ...
                textAnchor="middle"
              >
                Minutes from now (LSTM Forecasts)
              </text>
              <text
                x="5"
                y="50"
        ...
        {/* Command Advisor */}
        <div
          className={`md:col-span-1 p-5 rounded-xl border flex flex-col justify-between ${isDark ? "bg-[#0e162b] border-[#c9a227]/10" : "bg-white border-slate-200"}`}
        >
          <div>
            ...
            <p className="text-xs text-gray-400 mb-4">
              Generates localized crowd redirect and steward deployment
              parameters.
            </p>
          ...
          <div className="space-y-1.5 mt-4">
            <div className="text-[10px] font-bold text-[#abc0d8] uppercase">
              Recommended Action:
            </div>
            <button className="w-full text-left py-1.5 px-3.5 bg-slate-800 hover:bg-slate-700 rounded text-xs text-white transition-colors">
              Reroute incoming flows to Gate C
            </button>
            <button className="w-full text-left py-1.5 px-3.5 bg-slate-800 hover:bg-slate-700 rounded text-xs text-white transition-colors">
              Broadcast exit advisories to Sec 200
            </button>
          </div>
        </div>
      </div>
      ...
      >
        <h4 className="text-xs font-black text-[#c9a227] tracking-wider uppercase mb-3">
          Steward Incident Queue Log
        </h4>

        <div className="overflow-x-auto">
          <table
            className="w-full text-xs text-left"
            aria-label="Incident logs table"
          >
            <thead>
            ...
            <tbody>
              {state.incidents.map((inc) => (
                <tr
                  key={inc.id}
                  className={`border-b border-gray-700/10 hover:bg-slate-800/10 transition-colors ${inc.severity === "High" && inc.isNew ? "animate-red-card" : ""}`}
                >
                  ...
                  <td className="py-2.5 font-semibold text-amber-500">
                    {inc.status}
                  </td>
                  ...
                  <td className="py-2.5 text-right">
                    <div className="flex gap-2 justify-end">
                      <button
                        onClick={() =>
                          handleStatusUpdate(inc.id, "In Progress")
                        }
                      ...
                      <button
                        onClick={() => handleStatusUpdate(inc.id, "Completed")}
                        className="px-2 py-1 bg-green-900/40 text-green-400 rounded text-[10px] hover:bg-green-900/60"
                      >
                        Resolve
                      </button>
                    </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <nav
        ...
        aria-label="Volunteer Portal Subnavigation Options"
      >
        {tabs.map((t) => (
          <button
            key={t.id}
      ...
      <div className="min-h-[50vh]">
        ...
        {activeTab === "sop" && (
          <VolunteerTabSop isDark={isDark} isAccess={isAccess} />
        )}
        {activeTab === "report" && (
          <VolunteerTabReport
            dispatch={dispatch}
...
function VolunteerTabBriefing({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="space-y-4 text-xs leading-relaxed">
          <p>
            ...
            <span className="font-bold text-white">
              Section 200 Concourse Portal
            </span>
          </p>
          <p>
            Sector Occupancy Surge Metrics: Section 200 currently registers at
            94% capacity. Locals routing signals deployed to push overflow to
            Gate C exits.
          </p>
          <p>
            Seeding Access Needs: Wheelchair registration identified at Row 12,
            Seat 4. Ensure egress clear of queue barricades.
          </p>
        </div>
      </div>
      ...
      <div
        className={`p-4 rounded-xl border ${isDark ? "bg-[#0e162b] border-slate-800" : "bg-white border-slate-200"} text-xs font-mono space-y-1.5`}
      >
        <div className="font-bold text-[#c9a227]">BLE Position Coordinator</div>
        <div className="text-gray-400">
          Position grid tracking coordinates:{" "}
          <span className="text-white font-bold">
            X: 182 | Y: 74 (Sec 200 Portal)
          </span>
        </div>
        <div className="text-gray-400">
          System check-in status:{" "}
          <span className="text-green-500 font-bold">
            Automatic BLE Sync Completed
          </span>
        </div>
      </div>
    </div>
...
function VolunteerTabTasks({
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-bold uppercase tracking-wider text-[#abc0d8]">
        Assigned Task Checklist Board
      </h3>
      ...
      {state.tasks.map((task) => (
        ...
        >
          <div className="flex-1">
            ...
            <p
              className={`text-xs font-bold ${task.status === "completed" ? "line-through text-gray-400" : "text-white"}`}
            >
              {task.label}
            </p>
          </div>
...
function VolunteerTabSop({
  ...
  >([
    {
      id: "1",
      sender: "ai",
      text: "[SOP Help] Systems active. Enter keyword terms (e.g., lost child, medical aid, fire hazard) to pull real-time pgvector relevance scores and protocol listings.",
    },
  ]);
  const [inputVal, setInputVal] = useState("");
  ...
  return (
    <div
      className={`rounded-xl border p-5 space-y-4 flex flex-col h-[420px] ${isDark ? "bg-[#0e162b] border-[#c9a227]/10" : "bg-white border-slate-200"}`}
    >
      <div className="flex items-center gap-2 border-b border-gray-700/20 pb-3">
        <h3 className="text-sm font-bold">
          Standard Operating Procedures Assistant
        </h3>
        <GenAiBadge />
      </div>
      ...
      <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-thin">
        {messages.map((m) => (
          ...
          >
            <div
              className={`max-w-[80%] rounded-xl px-3 py-2 text-xs font-semibold leading-relaxed ${
                m.sender === "user"
                  ? "bg-[#001a4e] text-white rounded-br-none"
                  : "bg-slate-800/60 border border-slate-700/50 text-[#f3f6fa] rounded-bl-none"
              }`}
      ...
      <details className="text-[9.5px] border-t border-gray-700/20 pt-2 text-gray-400 font-mono">
        <summary className="cursor-pointer font-bold text-amber-500/70 hover:text-amber-400">
          pgvector RAG Embeddings Inspector
        </summary>
        <div className="mt-2 space-y-1 bg-slate-950 p-2 rounded border border-slate-800">
          <div>
            Prompt Vector Space:{" "}
            <span className="text-white">{"{sop_database_embeddings}"}</span>
          </div>
          <div>
            Retrieved Vector relevance score:{" "}
            <span className="text-green-400 font-bold">0.9124</span>
          </div>
          <div>
            pgvector matched segment:{" "}
            <span className="text-white font-bold">
              sop_protocol_14a_lost_child
            </span>
          </div>
        </div>
      </details>
...
function VolunteerTabReport({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!description.trim() || !location.trim()) {
      alert(
        "Please fill in both the description and location fields before submitting!",
      );
      return;
    }
  ...
  return (
    <div
      className={`p-5 rounded-xl border ${isDark ? "bg-[#0e162b] border-[#c9a227]/15" : "bg-white border-slate-200"}`}
    >
      <h3 className="text-sm font-bold uppercase tracking-wider mb-2 text-[#c9a227]">
        Report New Operational Incident
      </h3>
      <p className="text-xs text-gray-400 mb-4">
        Alerts sent here directly propagate into central operations queues in
        real time.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {successMsg && (
          <div className="text-xs text-green-500 font-bold text-center">
            ✓ Incident submitted successfully. Broadcasted to central logs.
          </div>
        )}
      </form>
    </div>
