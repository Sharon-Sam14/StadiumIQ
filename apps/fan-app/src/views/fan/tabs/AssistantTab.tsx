...
const RESPONSE_TREES: Record<Language, Record<string, string>> = {
  en: {
    default:
      "Hello! I'm your StadiumIQ AI Concierge for FIFA World Cup 2026 at MetLife Stadium. I can help with directions, food, transport, and match info. What do you need?",
    ...
    transport:
      "After the match, I recommend exiting via Gate C (lowest congestion: 2.4 min wait) and boarding NJ Transit at Meadowlands Station. Expected arrival at Newark Penn Station: 38 minutes. Train departs at 22:45.",
    toilet:
      "The nearest restrooms to your current estimated position (near Gate B) are on Level 2, North Concourse — approximately 90 meters. A family-accessible restroom is available on Level 1, Section 210.",
  },
  ar: {
    default:
      "مرحباً! أنا مساعد StadiumIQ الذكي لكأس العالم 2026. يمكنني مساعدتك في التنقل والطعام والمواصلات. كيف يمكنني مساعدتك؟",
    ...
    default_fallback:
      "سأحاول مساعدتك. هل يمكنك توضيح سؤالك؟ يمكنني مساعدتك في الطعام أو التنقل أو المواصلات.",
  },
  fr: {
    default:
      "Bonjour! Je suis votre assistant IA StadiumIQ pour la Coupe du Monde 2026. Comment puis-je vous aider avec la navigation, la restauration ou les transports?",
    ...
    default_fallback:
      "Je vais essayer de vous aider. Pouvez-vous préciser votre question?",
  },
  es: {
    default:
      "¡Hola! Soy tu asistente IA StadiumIQ para la Copa del Mundo 2026. ¿En qué puedo ayudarte con la navegación, comida o transporte?",
  ...
  pt: {
    default:
      "Olá! Sou seu assistente IA StadiumIQ para a Copa do Mundo 2026. Como posso ajudá-lo com navegação, comida ou transporte?",
...
const CONTEXT_CHUNKS = [
  {
    similarity: 0.912,
    content:
      "'Halal Bites: West Concourse, Section 112. Certified halal. Wait ~3 min.'",
    source: "venue_faq.md",
  },
  {
    similarity: 0.854,
    content:
      "'Gate A: High congestion zone (92%). AI recommends Gate B redirect.'",
    source: "crowd_telemetry_live",
  },
  {
    similarity: 0.733,
    content: "'NJ Transit departs Meadowlands Station 10 min post-match.'",
    source: "transport_schedule.json",
  },
  {
    similarity: 0.698,
    content:
      "'Section 212 accessible via Ramps B2, B4. Elevator at Gate B Level 1.'",
    source: "venue_faq.md",
  },
];
...
export const AssistantTab = React.memo(
  function AssistantTab(): React.JSX.Element {
    ...
    const handleSend = useCallback((): void => {
      ...
      callAiConcierge({ prompt: sanitized, sessionId: "session-pwa-fan" })
        .then((result: any) => {
          const reply =
            result.data?.text || "I'm sorry, I couldn't process your request.";
    ...
    return (
      <div className="flex flex-col h-full animate-fade-in gap-4">
        ...
        <div
          ...
          role="group"
          aria-label="Select language for AI assistant"
        >
        ...
        {showInspector && (
          <div className="rounded-xl border border-[var(--brand-gold)]/20 bg-[var(--brand-gold)]/5 p-4 space-y-2 text-[9px] font-mono overflow-y-auto max-h-40 animate-slide-up">
            <p className="text-[var(--brand-gold)] font-bold text-[10px]">
              SYSTEM PROMPT TEMPLATE
            </p>
            <p className="text-[var(--text-secondary)]">
              "You are a helpful FIFA World Cup 2026 concierge assistant for
              MetLife Stadium. Use retrieved venue context chunks (pgvector
              cosine similarity) to answer fan queries in their preferred
              language."
            </p>
            <p className="text-[var(--brand-gold)] font-bold text-[10px] mt-2">
              RETRIEVED VECTOR CONTEXTS
            </p>
            {CONTEXT_CHUNKS.map((chunk, i) => (
              <p
                key={i}
                className={`leading-relaxed ${chunk.similarity > 0.85 ? "text-green-400" : chunk.similarity > 0.75 ? "text-yellow-400" : "text-[var(--text-tertiary)]"}`}
              >
