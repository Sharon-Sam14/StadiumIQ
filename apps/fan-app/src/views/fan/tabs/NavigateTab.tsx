...
export const NavigateTab = React.memo(
  function NavigateTab(): React.JSX.Element {
      ...
      const interval = setInterval(() => {
        setZones((prev) =>
          prev.map((z) => ({
            ...
            density: Math.max(
              5,
              Math.min(99, z.density + Math.floor(Math.random() * 9) - 4),
            ),
          })),
        );
      }, 5000);
      ...
      <div className="space-y-4 animate-fade-in">
            ...
            >
              ...
              {zones.map((zone) => (
                <g key={zone.id} className="heatmap-zone">
                  ...
                  >
                    <title>
                      {zone.label}: {zone.density}% occupancy —{" "}
                      {densityToLabel(zone.density)}
                    </title>
                  </ellipse>
                </g>
              ))}
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            aria-label={
              showTelemetry
                ? "Hide BLE telemetry panel"
                : "Show BLE telemetry panel"
            }
          >
            {showTelemetry ? (
          ...
          <div className="rounded-xl border border-blue-500/20 bg-blue-500/5 overflow-hidden animate-slide-up">
            <div className="px-4 py-3 border-b border-blue-500/15 flex items-center justify-between">
              ...
              <span className="font-mono text-[9px] text-[var(--brand-gold)] font-bold">
                Pos: ({Math.round(bleTelemetry.estimated.x)},{" "}
                {Math.round(bleTelemetry.estimated.y)})
              </span>
            </div>
            <div className="p-3">
              <table
                className="w-full text-[10px] font-mono"
                aria-label="BLE beacon signal matrix"
              >
                <thead>
              ...
              <p className="text-[9px] text-[var(--text-tertiary)] mt-2.5 text-center">
                Triangulated via Bluetooth Low Energy (BLE) beacons using
                path-loss model: distance = 10^((-30 − RSSI) / 20)
              </p>
            </div>
          </div>
        )}
