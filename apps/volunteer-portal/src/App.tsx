...
export default function App() {
  const [activeTab, setActiveTab] = useState<
    "briefing" | "tasks" | "assistant" | "report"
  >("briefing");
  ...
  const [chatLogs, setChatLogs] = useState([
    {
      role: "assistant",
      text: "Welcome to the Volunteer Assistant, Jake. Ask me any procedural question (e.g. 'procedure for lost child' or 'first aid protocol').",
    },
  ]);
  ...
  const distA = Math.sqrt(
    (userTrueX - beaconA.x) ** 2 + (userTrueY - beaconA.y) ** 2,
  );
  const distB = Math.sqrt(
    (userTrueX - beaconB.x) ** 2 + (userTrueY - beaconB.y) ** 2,
  );
  const distC = Math.sqrt(
    (userTrueX - beaconC.x) ** 2 + (userTrueY - beaconC.y) ** 2,
  );
  ...
  const handleSendMessage = () => {
    ...
    setTimeout(() => {
      let reply =
        "Checking procedural manuals. For general queries, please contact your section supervisor.";
      if (
        userMsg.toLowerCase().includes("child") ||
        userMsg.toLowerCase().includes("lost")
      ) {
        reply =
        ...
        userMsg.toLowerCase().includes("medical") ||
        userMsg.toLowerCase().includes("aid")
      ) {
        reply =
  ...
  const handleSubmitReport = (e: React.FormEvent) => {
    ...
    alert(
      `Incident logged successfully! Reference ID: INC-${Math.floor(Math.random() * 900) + 100}`,
    );
  ...
  return (
    <div
      id="main-content"
      className="min-h-screen bg-bg-base text-text-primary flex flex-col items-center justify-start pb-20 select-none"
    >
      ...
      <div className="w-full max-w-[480px] min-h-screen flex flex-col bg-bg-surface/20 border-x border-border-subtle relative">
        ...
        <header className="h-16 px-4 border-b border-border-subtle flex justify-between items-center bg-bg-surface/50 backdrop-blur-md sticky top-0 z-20">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <span className="text-[8px] bg-brand-gold/10 text-brand-gold px-1.5 py-0.5 rounded font-bold uppercase ml-2 tracking-wider">
                Volunteer
              </span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <div className="text-right hidden xs:block">
              ...
              <p className="text-[8px] text-text-secondary uppercase tracking-wider">
                Fan Services (Sec 200)
              </p>
            </div>
        ...
        {activeBroadcast && (
          <div
            className={clsx(
              ...
              activeBroadcast.severity === "high" &&
                "bg-alert-danger/25 border-alert-danger/45 text-text-primary",
              activeBroadcast.severity === "medium" &&
                "bg-alert-warning/25 border-alert-warning/45 text-text-primary",
              activeBroadcast.severity === "low" &&
                "bg-alert-success/25 border-alert-success/45 text-text-primary",
            )}
          >
        ...
        <div className="flex-1 p-4 overflow-y-auto space-y-4">
            ...
            <div className="space-y-4 animate-fade-in">
              <div className="glass-panel p-5 rounded-md border border-brand-green-light/20 bg-gradient-to-br from-brand-green-deep/20 via-bg-surface/80 to-bg-surface">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="font-outfit font-extrabold text-xs tracking-wider text-brand-gold uppercase">
                    Your AI Pre-Shift Brief
                  </h3>
                </div>
                ...
                <ul className="list-disc list-inside space-y-2 text-xs text-text-secondary mt-3">
                  <li>
                    **Surge Alert:** Gate A is bottlenecked. Proactively
                    redirect incoming fans to Gate B.
                  </li>
                  <li>
                    **Languages:** Expect French & Wolof speakers from Match 82
                    attendees.
                  </li>
                  <li>
                    **Key Check:** Ensure wheelchair companion paths are clear
                    at Section 212.
                  </li>
                </ul>
          ...
          {activeTab === "tasks" && (
            <div className="space-y-3 animate-fade-in">
              <h3 className="font-outfit font-bold text-xs uppercase tracking-wider text-text-secondary px-1">
              ...
              {showInspector && (
                <div className="glass-panel p-4 rounded-md border border-brand-gold/30 bg-brand-gold/5 text-[9px] font-mono space-y-2 max-h-[140px] overflow-y-auto">
                  ...
                  <p className="text-text-secondary">
                    "You are a stadium volunteer assistant. Retrieve standard
                    operating procedures (SOPs) from pgvector."
                  </p>
                  <p className="text-brand-gold font-bold">
                    RETRIEVED SOP CHUNKS (pgvector Cosine Sim):
                  </p>
            ...
            <div className="space-y-4 animate-fade-in">
              <div className="glass-panel p-5 rounded-md border border-border-subtle bg-bg-surface/30">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="font-outfit font-extrabold text-xs uppercase tracking-wider">
                ...
                <form onSubmit={handleSubmitReport} className="space-y-4">
                  <div>
                    <label className="block text-[10px] text-text-secondary uppercase mb-1 font-bold">
                      Incident Type
                    </label>
                    <select
        ...
        >
          <button
            className={clsx(
              "flex flex-col items-center gap-1 text-[9px] font-semibold transition-colors duration-150",
              activeTab === "briefing"
                ? "text-brand-gold"
                : "text-text-secondary",
            )}
          ...
          <button
            className={clsx(
              "flex flex-col items-center gap-1 text-[9px] font-semibold transition-colors duration-150",
              activeTab === "tasks" ? "text-brand-gold" : "text-text-secondary",
            )}
